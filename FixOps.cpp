...
namespace {
    ...
    void CopyBlock(Image_T* dstImg, const Image_T* srcImg, int dx, int dy, int sx, int sy, int sw, int sh, int mode, typename Image_T::PixType key, float alpha)
    {
        ...
        cerr << "Copying to " << dx << ',' << dy << " from " << sx << ',' << sy << " size " << sw << 'x' << sh << " mode " << mode << " key (mode 2) " << key << " alpha (mode 4) " << alpha << '\n';
        CopyRect(*dstImg, *srcImg, sx, sy, dx, dy, sw, sh, mode, key, alpha);
    }

    template <class Pixel_T>
    inline bool isDark(const typename Pixel_T p, const typename Pixel_T::ElType threshold)
    {
                ...
                if(ringIsClear) {
                    for(int by = y - NC; by <= y + NC; by++) {
                        ...
                        for(int bx = x - NC; bx <= x + NC; bx++) {
                            ...
                            dstImg(bx, by) = typename Image_T::PixType(element_traits<typename Image_T::PixType::ElType>::one()); // For debugging just paint the center box white
                        }
                    }
                }
            }
    ...
    template <class Image_T>
    void TransposeImage(Image_T &Out, const Image_T &In)
    {
        int iw = In.w(), ih = In.h();
    ...
    template <class Image_T>
    void ContentAwareResize(Image_T &Out, const Image_T &In, const int dw, const int dh)
    {
        ...
        for(int y = 0; y < ih; y++) {
            for(int x = 0; x < iw - 1; x++) {
                PixEnergy(x, y) = (Abs<float, chan>(static_cast<FPT>(In(x, y)) - static_cast<FPT>(In(x + 1, y))).sum_chan());
            }
        ...
        for(int x = 0; x < iw; x++)
            CumPixEnergy(x, 0) = PixEnergy(x, 0);
        for(int y = 1; y < ih; y++) {
        ...
        for(int y = ih - 1; y >= 0; y--) {
            if(xi > 0)
                memcpy(&Out(0, y), &In(0, y), sizeof(Image_T::PixType) * xi);
            if(xi < iw - 1)
                memcpy(&Out(xi, y), &In(xi + 1, y), sizeof(Image_T::PixType) * (iw - xi - 1));
...
void DoCopyBlock(baseImage* curImg, string &blitFName, int dx, int dy, int sx, int sy, int sw, int sh, int mode, uc4Pixel key, float alpha)
{
    baseImage* blitImg = NULL;

    blitImg = LoadtImage(blitFName.c_str());
    cerr << "Copy: blitImg: " << blitFName << " size = " << blitImg->w_virtual() << "x" << blitImg->h_virtual() << " chan = " << blitImg->chan_virtual() << typeid(*blitImg).name() << '\n';
    ASSERT_R(typeid(*curImg) == typeid(*blitImg));

    if(dynamic_cast<const uc1Image*>(curImg)) {
        CopyBlock(dynamic_cast<uc1Image*>(curImg), dynamic_cast<const uc1Image*>(blitImg), dx, dy, sx, sy, sw, sh, mode, key, alpha);
    } else if(dynamic_cast<const uc3Image*>(curImg)) {
        CopyBlock(dynamic_cast<uc3Image*>(curImg), dynamic_cast<const uc3Image*>(blitImg), dx, dy, sx, sy, sw, sh, mode, key, alpha);
    } else if(dynamic_cast<const uc4Image*>(curImg)) {
        CopyBlock(dynamic_cast<uc4Image*>(curImg), dynamic_cast<const uc4Image*>(blitImg), dx, dy, sx, sy, sw, sh, mode, key, alpha);
    } else {
...
void DoFill(baseImage* curImg, float r, float g, float b, float a)
{
    ...
    } else if(uc1Image* uc1I = dynamic_cast<uc1Image*>(curImg)) {
        uc1I->fill(uc1Pixel(r));
    } else if(uc3Image* uc3I = dynamic_cast<uc3Image*>(curImg)) {
        uc3I->fill(uc3Pixel(r, g, b));
    } else if(uc4Image* uc4I = dynamic_cast<uc4Image*>(curImg)) {
...
void DoGradient(baseImage* curImg, int c, bool isVert, int minx, int maxx)
{
    ...
    // Modifies image in place
    if(curImg->size_element_virtual() > 1 || !curImg->is_integer_virtual())
        throw DMcError("Unsupported image type.\n");
...
// Should templatize this as tImage<tPixel<1234,ChanType> >
baseImage* DoChanConvert(baseImage* curImg, int nchan)
{
    cerr << "Convert: from " << curImg->chan_virtual() << " to uc" << nchan << "Image\n";

    if(nchan == 1) {
        if(uc3Image* uc = dynamic_cast<uc3Image*>(curImg)) {
            curImg = replaceImage(curImg, new uc1Image(*uc));
        } else if(uc4Image* uc = dynamic_cast<uc4Image*>(curImg)) {
            curImg = replaceImage(curImg, new uc1Image(*uc));
        }
    } else if(nchan == 3) {
        if(uc1Image* uc = dynamic_cast<uc1Image*>(curImg)) {
            curImg = replaceImage(curImg, new uc3Image(*uc));
        } else if(uc4Image* uc = dynamic_cast<uc4Image*>(curImg)) {
            curImg = replaceImage(curImg, new uc3Image(*uc));
        }
    } else if(nchan == 4) {
        if(uc1Image* uc = dynamic_cast<uc1Image*>(curImg)) {
            curImg = replaceImage(curImg, new uc4Image(*uc));
...
template <class Image_T>
void DoMatMul(Image_T& Img, float m0, float m1, float m2, float m3, float m4, float m5, float m6, float m7, float m8, float m9, float ma, float mb, float mc, float md, float me, float mf)
{
...
template void DoMatMul(f3Image& Img, float m0, float m1, float m2, float m3, float m4, float m5, float m6, float m7, float m8, float m9, float ma, float mb, float mc, float md, float me, float mf);
template void DoMatMul(f4Image& Img, float m0, float m1, float m2, float m3, float m4, float m5, float m6, float m7, float m8, float m9, float ma, float mb, float mc, float md, float me, float mf);
...
baseImage* CAResizeBranch(baseImage* curImg, int newWid, int newHgt)
{
...
baseImage* DoCAResize(baseImage* curImg, int newWid, int newHgt)
{
    ...
    while(curImg->w_virtual() > newWid || curImg->h_virtual() > newHgt) {
        for(int i = 0; i < ITERS_PER_DIM && curImg->w_virtual() > newWid; i++) {
            curImg = CAResizeBranch(curImg, newWid, curImg->h_virtual());
        }
        ...
        for(int i = 0; i < ITERS_PER_DIM && curImg->w_virtual() > newHgt; i++) {
            curImg = CAResizeBranch(curImg, newHgt, curImg->h_virtual());
        }
...
template <class Image_T>
void DoHorizFlatten(Image_T& dstImg, const Image_T& srcImg, const float a, const float b)
{
...
template <class Image_T>
void DoFlatten(Image_T& dstImg, const Image_T& srcImg, const float shrinkFac, const float biasFac)
{
    ...
    while(shrunkImg->w() >= 512 || shrunkImg->h() >= 512) {
        ...
        shrunkImg = replaceImage(shrunkImg, smallerImg);
    ...
    for(int i = 0; i < srcImg.size(); i++)
        csum += tPixel<double, Image_T::PixType::Chan>(srcImg[i]);
...
template <class Image_T>
void DoPrintTransect(Image_T& Img, const int y)
{
    cerr << "Transect: y " << y << '\n';

    for(int x = 0; x < Img.w(); x++) {
        cout << x << ',' << static_cast<float>(Img(x, y).r()) << endl;
    }
}
...
template <class Image_T>
void DoThreshold(Image_T& Img, const typename Image_T::PixType threshold)
{
...
template <class Image_T>
void DoOptimizeLinearMap(const Image_T& Img)
{
...
template <class Image_T>
void DoLinearMap(Image_T& Img, const f3Pixel p0, const f3Pixel p1)
{
...
template baseImage* replaceImage(baseImage*, baseImage*);
